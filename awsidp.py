...
class AwsIdp:
    ...
    def authenticate(self, username, password):
        ...
        auth_data = json.dumps(auth_request, sort_keys=False, ensure_ascii=False)
        auth_headers = {
        ...
        auth_response = self.session.post(self.url, data=auth_data, headers=auth_headers)
        ...
        challenge_data = json.dumps(challenge_request, sort_keys=False, ensure_ascii=False)
        challenge_headers = {
            "X-Amz-Target": "AWSCognitoIdentityProviderService.RespondToAuthChallenge",
            "Content-Type": "application/x-amz-json-1.1",
            "user-agent": "Mozilla/5.0 (Windows; U; Windows NT 6.1; en-GB; rv:1.9.2.13) Gecko/20101203 Firefox/3.6.13 (.NET CLR 3.5.30729)"
        }
        auth_response = self.session.post(self.url, data=challenge_data, headers=challenge_headers)
    ...
    def renew_token(self, refresh_token):
        ...
        refresh_request_data = json.dumps(refresh_request, sort_keys=False, ensure_ascii=False)
        refresh_response = self.session.post(self.url,
